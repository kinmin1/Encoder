...
//该函数用于找当前TB经过扫描后第一个非零系数的坐标（scanPosLast），通过坐标才能找到系数位置信息，即当前TB最后一个非零系数的位置。
...
{
	memset(coeffNum, 0, MLS_GRP_NUM * sizeof(*coeffNum));//为每一个大小为64字节的数组初值都赋值为0，因为最多CG的个数为64，所以数组大小才设为64.
	...
	do
	{
		const uint32_t cgIdx = (uint32_t)scanPosLast >> MLS_CG_SIZE;//CG的索引，按照扫描的顺序从第一个CG开始遍历，在CG内也是按照扫描的顺序遍历，直到找到最后一个非零系数。

		const uint32_t posLast = scan[scanPosLast++];//当前坐标对应系数的位置。

		const int curCoeff = coeff[posLast];//当前位置的系数的值。
		const uint32_t isNZCoeff = (curCoeff != 0);//判断该系数是否为非零系数。
		// get L1 sig map
...
void setupDCTPrimitives_c(EncoderPrimitives* p)
{
	...
	p->cu[BLOCK_4x4].count_nonzero = count_nonzero_c;
	p->cu[BLOCK_8x8].count_nonzero = count_nonzero_c;
	p->cu[BLOCK_16x16].count_nonzero = count_nonzero_c;
	p->cu[BLOCK_32x32].count_nonzero = count_nonzero_c;

	p->cu[BLOCK_4x4].copy_cnt = copy_count;
	p->cu[BLOCK_8x8].copy_cnt = copy_count;
	p->cu[BLOCK_16x16].copy_cnt = copy_count;
	p->cu[BLOCK_32x32].copy_cnt = copy_count;

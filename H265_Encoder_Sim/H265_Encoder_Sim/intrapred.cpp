...
void intraFilter(const pixel* samples, pixel* filtered, int tuSize) /* 1:2:1 filtering of left and top reference samples */
{
...
void intra_pred_dc_c(pixel* dst, intptr_t dstStride, const pixel* srcPix, int dirMode, int bFilter, int width)
{
	...
	//printf("dcVal = %d",dcVal);
	for (k = 0; k < width; k++)
		for (l = 0; l < width; l++)
		{
			dst[k * dstStride + l] = (pixel)dcVal;
			//printf("dst[k * dstStride + l] = %d\n",dst[k * dstStride + l]);
		}
...
const char angleTable[17] = { -32, -26, -21, -17, -13, -9, -5, -2, 0, 2, 5, 9, 13, 17, 21, 26, 32 };
const short invAngleTable[8] = { 4096, 1638, 910, 630, 482, 390, 315, 256 };
...
int angle = angleTable[8 + angleOffset];
	...
	if (!angle)
	{
		...
		for (int y = 0; y < width; y++)
		dst[y * dstStride] = x265_clip((top + ((srcPix[width2 + 1 + y] - topLeft) >> 1))&0xffff);
		//dst[y * dstStride] =min((1 << X265_DEPTH) - 1,max(0,((srcPix[width2 + 1 + y] - topLeft) >> 1)));
		}
	}
...
void all_angs_pred_c(pixel *dest, pixel *refPix, pixel *filtPix, int bLuma, int log2Size)
{
	...
	for (int mode = 2; mode <= 34; mode++)
	{
		...
		intra_pred_ang_c(out, size, srcPix, mode, bLuma, size);
...
{
	p->cu[BLOCK_4x4].intra_filter = intraFilter;
	p->cu[BLOCK_8x8].intra_filter = intraFilter;
	p->cu[BLOCK_16x16].intra_filter = intraFilter;
	p->cu[BLOCK_32x32].intra_filter = intraFilter;

	p->cu[BLOCK_4x4].intra_pred[PLANAR_IDX] = planar_pred_c;
	p->cu[BLOCK_8x8].intra_pred[PLANAR_IDX] = planar_pred_c;
	p->cu[BLOCK_16x16].intra_pred[PLANAR_IDX] = planar_pred_c;
	p->cu[BLOCK_32x32].intra_pred[PLANAR_IDX] = planar_pred_c;

	p->cu[BLOCK_4x4].intra_pred[DC_IDX] = intra_pred_dc_c;
	p->cu[BLOCK_8x8].intra_pred[DC_IDX] = intra_pred_dc_c;
	p->cu[BLOCK_16x16].intra_pred[DC_IDX] = intra_pred_dc_c;
	p->cu[BLOCK_32x32].intra_pred[DC_IDX] = intra_pred_dc_c;

	for (int i = 2; i < NUM_INTRA_MODE; i++)
	{
		p->cu[BLOCK_4x4].intra_pred[i] = intra_pred_ang_c;
		p->cu[BLOCK_8x8].intra_pred[i] = intra_pred_ang_c;
		p->cu[BLOCK_16x16].intra_pred[i] = intra_pred_ang_c;
		p->cu[BLOCK_32x32].intra_pred[i] = intra_pred_ang_c;
	}

	p->cu[BLOCK_4x4].intra_pred_allangs = all_angs_pred_c;
	p->cu[BLOCK_8x8].intra_pred_allangs = all_angs_pred_c;
	p->cu[BLOCK_16x16].intra_pred_allangs = all_angs_pred_c;
	p->cu[BLOCK_32x32].intra_pred_allangs = all_angs_pred_c;
}

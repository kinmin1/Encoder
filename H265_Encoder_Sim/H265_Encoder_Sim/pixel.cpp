...
//待优化
int sse_int16_t(const int16_t* pix1, int stride_pix1, const int16_t* pix2, int stride_pix2, int lx, int ly)
{
	int sum = 0;
	int tmp;
...
{
	sum2_t s = ((a >> (BITS_PER_SUM - 1)) & (((sum2_t)1 << BITS_PER_SUM) + 1)) * 65535;

	return (a + s) ^ s;
}

int satd_4x4(const pixel* pix1, intptr_t stride_pix1, const pixel* pix2, intptr_t stride_pix2, int w, int h)
{
	sum2_t tmp[4][2];
	...
	for (int i = 0; i < 2; i++)
	{
		...
		sum += (a0 & 0xffff) + (a0 >> BITS_PER_SUM);
	}
	return (int)(sum >> 1);
}

static int satd_4x4_int16_t(const int16_t* pix1, intptr_t stride_pix1)
{
	int tmp[4][4];
	int s01, s23, d01, d23;
	int satd = 0;
...
int satd_8x4(const pixel* pix1, intptr_t stride_pix1, const pixel* pix2, intptr_t stride_pix2, int w, int h)
{
	sum2_t tmp[4][4];
	sum2_t a0, a1, a2, a3;
	sum2_t sum = 0;
	...
	return ((sum & 0xffff) + (sum >> BITS_PER_SUM)) >> 1;
}

// calculate satd in blocks of 4x4
int satd4(const pixel* pix1, intptr_t stride_pix1, const pixel* pix2, intptr_t stride_pix2, int w, int h)
{
	int satd = 0;
	...
	for (int row = 0; row < h; row += 4)
		for (int col = 0; col < w; col += 4)
			...
			pix2 + row * stride_pix2 + col, stride_pix2, 4, 4);

	return satd;
}

// calculate satd in blocks of 8x4
int satd8(const pixel* pix1, intptr_t stride_pix1, const pixel* pix2, intptr_t stride_pix2, int w, int h)
{
	int satd = 0;
	...
	for (int row = 0; row < h; row += 4)
		for (int col = 0; col < w; col += 8)
			...
			pix2 + row * stride_pix2 + col, stride_pix2, 8, 4);

	return satd;
}

int _sa8d_8x8(const pixel* pix1, intptr_t i_pix1, const pixel* pix2, intptr_t i_pix2)
{
	//sum2_t tmp[8][4];
	...
	for (int i = 0; i < 4; i++)
	{
		...
		//sum += (sum_t)b0 + (b0 >> BITS_PER_SUM);ECS强制转换有问题，用位操作代替
		sum += (b0&0xffff) + (b0 >> BITS_PER_SUM);
	}
	return (int)sum;
}

int sa8d_8x8(const pixel* pix1, intptr_t i_pix1, const pixel* pix2, intptr_t i_pix2, int w, int h)
{
	return (int)((_sa8d_8x8(pix1, i_pix1, pix2, i_pix2) + 2) >> 2);
}

//inline//待优化**
int _sa8d_8x8_1(const int16_t* pix1, intptr_t i_pix1)
{
	int tmp[8][8];
...
int sa8d_8x8_1(const int16_t* pix1, intptr_t i_pix1)
{
	return (int)((_sa8d_8x8_1(pix1, i_pix1) + 2) >> 2);
}
int sa8d_88(const pixel* pix1, intptr_t i_pix1, const pixel* pix2, intptr_t i_pix2);//_sa8d_8x8
int sa8d_16x16(const pixel* pix1, intptr_t i_pix1, const pixel* pix2, intptr_t i_pix2, int w, int h)
{
...
int sa8d8(const pixel* pix1, intptr_t i_pix1, const pixel* pix2, intptr_t i_pix2, int w, int h)
{
	int cost = 0;

	for (int y = 0; y < h; y += 8)
		for (int x = 0; x < w; x += 8)
			cost += sa8d_8x8(pix1 + i_pix1 * y + x, i_pix1, pix2 + i_pix2 * y + x, i_pix2, 8, 8);

	return cost;
}

// Calculate sa8d in blocks of 16x16
int sa8d16(const pixel* pix1, intptr_t i_pix1, const pixel* pix2, intptr_t i_pix2, int w, int h)
{
	int cost = 0;

	for (int y = 0; y < h; y += 16)
		for (int x = 0; x < w; x += 16)
			cost += sa8d_16x16(pix1 + i_pix1 * y + x, i_pix1, pix2 + i_pix2 * y + x, i_pix2, 16, 16);

	return cost;
}
//待优化zhenjian
int pixel_ssd_s_c(const int16_t* a, intptr_t dstride, int size)
{
	int sum = 0;
...
void blockfill_s_c(int16_t* dst, intptr_t dstride, int16_t val, int size)
{
for (int y = 0; y < size; y++)
for (int x = 0; x < size; x++)
dst[y * dstride + x] = val;
...
void cpy2Dto1D_shl(int16_t* dst, int16_t* src, intptr_t srcStride, int shift, int size)
{
...
void cpy2Dto1D_shr(int16_t* dst, int16_t* src, intptr_t srcStride, int shift, int size)
{
	X265_CHECK(((intptr_t)dst & 15) == 0, "dst alignment error\n");
...
void cpy1Dto2D_shl(int16_t* dst, int16_t* src, intptr_t dstStride, int shift, int size)
{
...
void cpy1Dto2D_shr(int16_t* dst, int16_t* src, intptr_t dstStride, int shift, int size)
{
	X265_CHECK((((intptr_t)dst | (dstStride * sizeof(*dst))) & 15) == 0 || size == 4, "dst alignment error\n");
...
void weight_pp_c(const pixel* src, pixel* dst, intptr_t stride, int width, int height, int w0, int round, int shift, int offset)
{
	...
	for (y = 0; y <= height - 1; y++)
	{
		for (x = 0; x <= width - 1;)
		{
			// simulating pixel to int16_t conversion
...
void pixelavg_pp(pixel* dst, intptr_t dstride, const pixel* src0, intptr_t sstride0, const pixel* src1, intptr_t sstride1, int weight, int lx, int ly)
{
	for (int y = 0; y < ly; y++)
	{
		for (int x = 0; x < lx; x++)
...
uint64_t pixel_var(const pixel* pix, intptr_t i_stride, int size)
{
...
//待优化
int psyCost_pp(const pixel* source, intptr_t sstride, const pixel* recon, intptr_t rstride, int size)
{
	...
	{
		int dim = 1 << (size + 2);
		unsigned int totEnergy = 0;
		for (int i = 0; i < dim; i += 8)
		{
			for (int j = 0; j < dim; j += 8)
			{
				/* AC energy, measured by sa8d (AC + DC) minus SAD (DC) */
				int sourceEnergy = sa8d_8x8(source + i * sstride + j, sstride, zeroBuf, 0, 8, 8) -
					(sad(source + i * sstride + j, sstride, zeroBuf, 0, 8, 8) >> 2);
				int reconEnergy = sa8d_8x8(recon + i * rstride + j, rstride, zeroBuf, 0, 8, 8) -
					(sad(recon + i * rstride + j, rstride, zeroBuf, 0, 8, 8) >> 2);
	...
	else
	{
		...
		int sourceEnergy = satd_4x4(source, sstride, zeroBuf, 0, 4, 4) - (sad(source, sstride, zeroBuf, 0, 4, 4) >> 2);
		int reconEnergy = satd_4x4(recon, rstride, zeroBuf, 0, 4, 4) - (sad(recon, rstride, zeroBuf, 0, 4, 4) >> 2);
		return abs(sourceEnergy - reconEnergy);
	}
}
//待优化
int psyCost_ss(const int16_t* source, intptr_t sstride, const int16_t* recon, intptr_t rstride, int size)
{
	...
	if (size)
	{
		...
		for (int i = 0; i < dim; i += 8)
		{
			...
			{
				/* AC energy, measured by sa8d (AC + DC) minus SAD (DC) */
				int sourceEnergy = sa8d_8x8_1(source + i * sstride + j, sstride) -
					(sad_int16_t(source + i * sstride + j, sstride, zeroBuf, 0, 8, 8) >> 2);
				int reconEnergy = sa8d_8x8_1(recon + i * rstride + j, rstride) -
					(sad_int16_t(recon + i * rstride + j, rstride, zeroBuf, 0, 8, 8) >> 2);
	...
	{
		/* 4x4 is too small for sa8d */
		int sourceEnergy = satd_4x4_int16_t(source, sstride) - (sad_int16_t(source, sstride, zeroBuf, 0, 4, 4) >> 2);
		int reconEnergy = satd_4x4_int16_t(recon, rstride) - (sad_int16_t(recon, rstride, zeroBuf, 0, 4, 4) >> 2);
		return abs(sourceEnergy - reconEnergy);
	}
}
...
//待优化inter
void blockcopy_ss_c(int16_t* a, intptr_t stridea, const int16_t* b, intptr_t strideb, int bx, int by)
{
	for (int y = 0; y < by; y++)
	{
		for (int x = 0; x < bx; x++)
...
void blockcopy_sp_c(pixel* a, intptr_t stridea, const int16_t* b, intptr_t strideb, int bx, int by)
{
	for (int y = 0; y < by; y++)
	{
		for (int x = 0; x < bx; x++)
...
void blockcopy_ps_c(int16_t* a, intptr_t stridea, const pixel* b, intptr_t strideb, int bx, int by)
{
	for (int y = 0; y < by; y++)
	{
		for (int x = 0; x < bx; x++)
...
//待优化zhenjian
void pixel_sub_ps_c(int16_t* a, intptr_t dstride, const pixel* b0, const pixel* b1, intptr_t sstride0, intptr_t sstride1, int bx, int by)
{
	for (int y = 0; y < by; y++)
	{
		for (int x = 0; x < bx; x++)
...
//待优化
void pixel_add_ps_c(pixel* a, intptr_t dstride, const pixel* b0, const int16_t* b1, intptr_t sstride0, intptr_t sstride1, int bx, int by)
{
	for (int y = 0; y < by; y++)
	{
		for (int x = 0; x < bx; x++)
...
void addAvg(const int16_t* src0, const int16_t* src1, pixel* dst, intptr_t src0Stride, intptr_t src1Stride, intptr_t dstStride, int bx, int by)
{
	int shiftNum, offset;
...
#define LUMA_PU(W, H) \
    ...
    p->pu[LUMA_ ## W ## x ## H].pixelavg_pp = pixelavg_pp;

#define LUMA_CU(W, H) \
    p->cu[BLOCK_ ## W ## x ## H].sub_ps        = pixel_sub_ps_c; \
    p->cu[BLOCK_ ## W ## x ## H].add_ps        = pixel_add_ps_c; \
    p->cu[BLOCK_ ## W ## x ## H].copy_sp       = blockcopy_sp_c; \
    p->cu[BLOCK_ ## W ## x ## H].copy_ps       = blockcopy_ps_c; \
    p->cu[BLOCK_ ## W ## x ## H].copy_ss       = blockcopy_ss_c; \
    p->cu[BLOCK_ ## W ## x ## H].blockfill_s   = blockfill_s_c;  \
    p->cu[BLOCK_ ## W ## x ## H].cpy2Dto1D_shl = cpy2Dto1D_shl; \
    p->cu[BLOCK_ ## W ## x ## H].cpy2Dto1D_shr = cpy2Dto1D_shr; \
    p->cu[BLOCK_ ## W ## x ## H].cpy1Dto2D_shl = cpy1Dto2D_shl; \
    p->cu[BLOCK_ ## W ## x ## H].cpy1Dto2D_shr = cpy1Dto2D_shr; \
    p->cu[BLOCK_ ## W ## x ## H].psy_cost_pp   = psyCost_pp; \
    p->cu[BLOCK_ ## W ## x ## H].psy_cost_ss   = psyCost_ss; \
    p->cu[BLOCK_ ## W ## x ## H].transpose     = transpose; \
    p->cu[BLOCK_ ## W ## x ## H].ssd_s         = pixel_ssd_s_c; \
...
//	p->cu[BLOCK_ ## W ## x ## H].var           = pixel_var; \
	...
	p->pu[LUMA_8x8].satd = satd8;
	p->pu[LUMA_8x4].satd = satd_8x4;
	p->pu[LUMA_4x8].satd = satd4;
	p->pu[LUMA_16x16].satd = satd8;
	p->pu[LUMA_16x8].satd = satd8;
	p->pu[LUMA_8x16].satd = satd8;
	p->pu[LUMA_16x12].satd = satd8;
	p->pu[LUMA_12x16].satd = satd4;
	p->pu[LUMA_16x4].satd = satd8;
	p->pu[LUMA_4x16].satd = satd4;
	p->pu[LUMA_32x32].satd = satd8;
	p->pu[LUMA_32x16].satd = satd8;
	p->pu[LUMA_16x32].satd = satd8;
	p->pu[LUMA_32x24].satd = satd8;
	p->pu[LUMA_24x32].satd = satd8;
	p->pu[LUMA_32x8].satd = satd8;
	p->pu[LUMA_8x32].satd = satd8;
	p->pu[LUMA_64x64].satd = satd8;
	p->pu[LUMA_64x32].satd = satd8;
	p->pu[LUMA_32x64].satd = satd8;
	p->pu[LUMA_64x48].satd = satd8;
	p->pu[LUMA_48x64].satd = satd8;
	p->pu[LUMA_64x16].satd = satd8;
	p->pu[LUMA_16x64].satd = satd8;
	...
	p->cu[BLOCK_32x32].sa8d = sa8d16;
	p->cu[BLOCK_64x64].sa8d = sa8d16;

#define CHROMA_PU_420(W, H) \
    p->chroma[X265_CSP_I420].pu[CHROMA_420_ ## W ## x ## H].addAvg  = addAvg;         \
    p->chroma[X265_CSP_I420].pu[CHROMA_420_ ## W ## x ## H].copy_pp = blockcopy_pp_c; \
	...
	p->chroma[X265_CSP_I420].pu[CHROMA_420_8x8].satd = satd8;
	p->chroma[X265_CSP_I420].pu[CHROMA_420_16x16].satd = satd8;
	p->chroma[X265_CSP_I420].pu[CHROMA_420_32x32].satd = satd8;
	...
	p->chroma[X265_CSP_I420].pu[CHROMA_420_4x8].satd = satd4;
	p->chroma[X265_CSP_I420].pu[CHROMA_420_16x8].satd = satd8;
	p->chroma[X265_CSP_I420].pu[CHROMA_420_8x16].satd = satd8;
	p->chroma[X265_CSP_I420].pu[CHROMA_420_32x16].satd = satd8;
	p->chroma[X265_CSP_I420].pu[CHROMA_420_16x32].satd = satd8;
	...
	p->chroma[X265_CSP_I420].pu[CHROMA_420_16x12].satd = satd4;
	p->chroma[X265_CSP_I420].pu[CHROMA_420_12x16].satd = satd4;
	p->chroma[X265_CSP_I420].pu[CHROMA_420_16x4].satd = satd4;
	p->chroma[X265_CSP_I420].pu[CHROMA_420_4x16].satd = satd4;
	p->chroma[X265_CSP_I420].pu[CHROMA_420_32x24].satd = satd8;
	p->chroma[X265_CSP_I420].pu[CHROMA_420_24x32].satd = satd8;
	p->chroma[X265_CSP_I420].pu[CHROMA_420_32x8].satd = satd8;
	p->chroma[X265_CSP_I420].pu[CHROMA_420_8x32].satd = satd8;

#define CHROMA_CU_420(W, H) \
    p->chroma[X265_CSP_I420].cu[BLOCK_420_ ## W ## x ## H].copy_sp = blockcopy_sp_c; \
    p->chroma[X265_CSP_I420].cu[BLOCK_420_ ## W ## x ## H].copy_ps = blockcopy_ps_c; \
    p->chroma[X265_CSP_I420].cu[BLOCK_420_ ## W ## x ## H].copy_ss = blockcopy_ss_c; \
    p->chroma[X265_CSP_I420].cu[BLOCK_420_ ## W ## x ## H].sub_ps = pixel_sub_ps_c;  \
    p->chroma[X265_CSP_I420].cu[BLOCK_420_ ## W ## x ## H].add_ps = pixel_add_ps_c;\
    p->chroma[X265_CSP_I420].cu[BLOCK_420_ ## W ## x ## H].sse_pp  = sse_pixel;
	...
	p->chroma[X265_CSP_I420].cu[BLOCK_16x16].sa8d = sa8d8;
	p->chroma[X265_CSP_I420].cu[BLOCK_32x32].sa8d = sa8d16;
	p->chroma[X265_CSP_I420].cu[BLOCK_64x64].sa8d = sa8d16;

#define CHROMA_PU_422(W, H) \
    p->chroma[X265_CSP_I422].pu[CHROMA_422_ ## W ## x ## H].addAvg  = addAvg;         \
    p->chroma[X265_CSP_I422].pu[CHROMA_422_ ## W ## x ## H].copy_pp = blockcopy_pp_c; \
	...
	p->chroma[X265_CSP_I422].pu[CHROMA_422_4x8].satd = satd4;
	p->chroma[X265_CSP_I422].pu[CHROMA_422_8x16].satd = satd8;
	p->chroma[X265_CSP_I422].pu[CHROMA_422_16x32].satd = satd8;
	p->chroma[X265_CSP_I422].pu[CHROMA_422_32x64].satd = satd8;

	p->chroma[X265_CSP_I422].pu[CHROMA_422_4x4].satd = satd_4x4;
	p->chroma[X265_CSP_I422].pu[CHROMA_422_2x8].satd = NULL;
	p->chroma[X265_CSP_I422].pu[CHROMA_422_8x8].satd = satd8;
	p->chroma[X265_CSP_I422].pu[CHROMA_422_4x16].satd = satd4;
	p->chroma[X265_CSP_I422].pu[CHROMA_422_16x16].satd = satd8;
	p->chroma[X265_CSP_I422].pu[CHROMA_422_8x32].satd = satd8;
	p->chroma[X265_CSP_I422].pu[CHROMA_422_32x32].satd = satd8;
	p->chroma[X265_CSP_I422].pu[CHROMA_422_16x64].satd = satd8;

	p->chroma[X265_CSP_I422].pu[CHROMA_422_8x12].satd = satd4;
	p->chroma[X265_CSP_I422].pu[CHROMA_422_6x16].satd = NULL;
	p->chroma[X265_CSP_I422].pu[CHROMA_422_8x4].satd = satd4;
	p->chroma[X265_CSP_I422].pu[CHROMA_422_2x16].satd = NULL;
	p->chroma[X265_CSP_I422].pu[CHROMA_422_16x24].satd = satd8;
	p->chroma[X265_CSP_I422].pu[CHROMA_422_12x32].satd = satd4;
	p->chroma[X265_CSP_I422].pu[CHROMA_422_16x8].satd = satd8;
	p->chroma[X265_CSP_I422].pu[CHROMA_422_4x32].satd = satd4;
	p->chroma[X265_CSP_I422].pu[CHROMA_422_32x48].satd = satd8;
	p->chroma[X265_CSP_I422].pu[CHROMA_422_24x64].satd = satd8;
	p->chroma[X265_CSP_I422].pu[CHROMA_422_32x16].satd = satd8;
	p->chroma[X265_CSP_I422].pu[CHROMA_422_8x64].satd = satd8;

#define CHROMA_CU_422(W, H) \
    p->chroma[X265_CSP_I422].cu[BLOCK_422_ ## W ## x ## H].copy_sp = blockcopy_sp_c; \
    p->chroma[X265_CSP_I422].cu[BLOCK_422_ ## W ## x ## H].copy_ps = blockcopy_ps_c; \
    p->chroma[X265_CSP_I422].cu[BLOCK_422_ ## W ## x ## H].copy_ss = blockcopy_ss_c; \
    p->chroma[X265_CSP_I422].cu[BLOCK_422_ ## W ## x ## H].sub_ps = pixel_sub_ps_c; \
    p->chroma[X265_CSP_I422].cu[BLOCK_422_ ## W ## x ## H].add_ps = pixel_add_ps_c;\
    p->chroma[X265_CSP_I422].cu[BLOCK_422_ ## W ## x ## H].sse_pp  = sse_pixel;
	...
	p->chroma[X265_CSP_I422].cu[BLOCK_16x16].sa8d = sa8d8;
	p->chroma[X265_CSP_I422].cu[BLOCK_32x32].sa8d = sa8d16;
	p->chroma[X265_CSP_I422].cu[BLOCK_64x64].sa8d = sa8d16;
